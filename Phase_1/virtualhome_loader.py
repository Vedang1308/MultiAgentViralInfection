...
def run_virtualhome_baselines():
    env_name = "VirtualHome"
    total_tasks = 100
    checkpoint_file = "Phase_1/baselines/virtualhome/virtualhome_checkpoint.json"
    output_file = "Phase_1/baselines/virtualhome/virtualhome_golden_path.json"
    ...
    for task_id in range(start_task + 1, total_tasks + 1):
        # Simulate VirtualHome task execution
        # 1. Deploy male_1 and female_1
        # 2. Interact via shared knowledge graph using [action] <object_1> <id_1> syntax
        
        epistemic_trust_t = 2  # Timestep where Agent 0 writes structural update and Agent 1 pulls it
